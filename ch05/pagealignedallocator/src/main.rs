...
unsafe impl Allocator for PageAlignedAllocator {
  fn allocate(&self, layout: Layout) -> Result<ptr::NonNull<[u8]>, AllocError> {
    ...
    let aft_protected_region = unsafe {
      std::slice::from_raw_parts_mut(out.add(aft_protected_region_offset) as *mut u8, pagesize)
    };
  ...
  unsafe fn deallocate(&self, ptr: ptr::NonNull<u8>, layout: Layout) {
    ...
    let aft_protected_region =
      std::slice::from_raw_parts_mut(ptr.add(aft_protected_region_offset) as *mut u8, pagesize);
